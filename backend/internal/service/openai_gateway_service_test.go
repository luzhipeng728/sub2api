...
func TestOpenAISelectAccountWithLoadAwareness_PrefersUnderusedWithinLoadSlack(t *testing.T) {
	groupID := int64(1)
	recent := time.Now()
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1, LastUsedAt: &recent},
			{ID: 2, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1},
		},
	}
	concurrencyCache := stubConcurrencyCache{
	...
	svc.openaiAccountStats.reportAttempt(1, time.Now())
	...
	if selection == nil || selection.Account == nil || selection.Account.ID != 2 {
		t.Fatalf("expected underused account 2, got %#v", selection)
	}
}

func TestOpenAISelectAccountWithLoadAwareness_SoftErrorPenaltyDoesNotExcludeAccount(t *testing.T) {
	groupID := int64(1)
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1},
	...
	svc.openaiAccountStats.report(1, false, nil)
	ttft := 120
	svc.openaiAccountStats.report(2, true, &ttft)
	...
	if selection == nil || selection.Account == nil || selection.Account.ID != 1 {
		t.Fatalf("expected penalized account 1 to remain selectable after preferred account is full, got %#v", selection)
	}
}

func TestOpenAIReportScheduleResultRecordsStatsWhenAdvancedSchedulerDisabled(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	svc := &OpenAIGatewayService{}
	ttft := 180

	svc.ReportOpenAIAccountScheduleResult(88, true, &ttft)

	if svc.openaiAccountStats == nil {
		t.Fatalf("expected runtime stats to be initialized")
	}
	errorRate, gotTTFT, hasTTFT := svc.openaiAccountStats.snapshot(88)
